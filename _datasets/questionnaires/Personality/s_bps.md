---
layout: dataset
title: BPS – Boredom Proneness Scale
modality:
  - questionnaires
keywords:
  - Emotion
  - Mental health
site:
  - Aarhus
contact:
summary: A self-report questionnaire measuring individual differences in the tendency to experience boredom. The original scale contains 28 items rated on a 7-point Likert scale, though shorter versions also exist. The BPS assesses both internal and external stimulation deficits.
participants:
citation: "Farmer, R., & Sundberg, N. D. (1986). Boredom proneness: The development and correlates of a new scale. Journal of Personality Assessment, 50(1), 4-17."
doi: 10.1207/s15327752jpa5001_2
---

{% include JB/setup %}

#### Detailed description

The Boredom Proneness Scale (BPS) is a self-report measure designed to assess an individual's tendency to experience boredom. Boredom proneness reflects a stable personality trait characterized by difficulty engaging with one's environment and maintaining interest in activities.

The scale was developed to measure both internal (difficulty with attention and engagement) and external (lack of environmental stimulation) factors contributing to the experience of boredom.

**Administration**: Respondents rate each item on a 7-point Likert scale ranging from "highly disagree" to "highly agree."

**Note**: The original BPS has shown inconsistent factor structure across validation studies, with different studies identifying between 2 and 7 factors. Shorter versions have been developed to address these psychometric concerns.

#### Scoring

**Total Score**: Sum of all items (some reverse-scored), with higher scores indicating greater proneness to boredom.

The BPS has been found to correlate with:
- Difficulties with attention and concentration
- Sensation seeking
- Depression and negative affect
- Lower levels of life satisfaction

#### Related publications

Farmer, R., & Sundberg, N. D. (1986). Boredom proneness: The development and correlates of a new scale. Journal of Personality Assessment, 50(1), 4-17. https://doi.org/10.1207/s15327752jpa5001_2

Struk, A. A., Carriere, J. S. A., Cheyne, J. A., & Danckert, J. (2017). A Short Boredom Proneness Scale: Development and psychometric properties. Assessment, 24(3), 346-359. https://doi.org/10.1177/1073191115609996

Vodanovich, S. J., Wallace, J. C., & Kass, S. J. (2005). A confirmatory approach to the factor structure of the Boredom Proneness Scale: Evidence for a two-factor short form. Journal of Personality Assessment, 85(3), 295-303.
