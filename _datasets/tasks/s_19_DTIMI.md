...
* The experimental paradigm of Experiment 1. The display on the top left with the face and the outline of a square around it is associated with two different tasks (bottom row): the participant either has to indicate the gender of the face or rate the visibility of the square positioned around the face. In the single task condition (see methods) all these tasks are trained independently, but in the main experimental phase (dual-task condition) the participant has to be prepared for all these tasks. In the dual-task condition the participants had to perform the face task on 90% of trials, thus ensuring that attention is diverted away from the faint square stimulus. In the critical trials (top right) there was no square, but the participants were nevertheless prompted to rate the visibility of the absent square and leaving only the facial area visible.
